...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # 모든 도메인에서의 접근을 허용 (테스트용)
...
async def check_diary(data: DiaryInput):
    ...
    try:
        # Render의 Environment 메뉴에 저장한 비밀키를 직접 명확하게 꺼내옵니다.
        api_key_from_env = os.environ.get("GEMINI_API_KEY")
        
        # 최신 구글 Gemini Client에 발급받은 키를 주입합니다.
        client = genai.Client(api_key=api_key_from_env)
        
        # 구글의 가장 최신 기본 모델인 gemini-2.5-flash 사용
